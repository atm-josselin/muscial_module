...
/**
 * 	\defgroup   musical     Module Musical
 *  \brief      Musical module descriptor.
 *
 *  \file       htdocs/musical/core/modules/modMusical.class.php
 *  \ingroup    musical
 *  \brief      Description and activation file for module Musical
 */
include_once DOL_DOCUMENT_ROOT .'/core/modules/DolibarrModules.class.php';


/**
 *  Description and activation class for module Musical
 */
class modMusical extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		// Module label (no space allowed), used if translation string 'ModuleMusicalName' not found (Musical is name of module).
		$this->name = preg_replace('/^mod/i','',get_class($this));
		// Module description, used if translation string 'ModuleMusicalDesc' not found (Musical is name of module).
		$this->description = "MusicalDescription";
		...
		// Example: this->dirs = array("/musical/temp","/musical/subdir");
		$this->dirs = array("/musical/temp");

		// Config pages. Put here list of php page, stored into musical/admin directory, to use to setup module.
		$this->config_page_url = array("setup.php@musical");
		...
		$this->langfiles = array("musical@musical");
		//$this->phpmin = array(5,4);					// Minimum version of PHP required by module
		$this->need_dolibarr_version = array(4,0);		// Minimum version of Dolibarr required by module
		$this->warnings_activation = array();			// Warning to show when we activate module. array('always'='text') or array('FR'='textfr','ES'='textes'...)
		$this->warnings_activation_ext = array();		// Warning to show when we activate an external module. array('always'='text') or array('FR'='textfr','ES'='textes'...)
		//$this->automatic_activation = array('FR'=>'MusicalWasAutomaticallyActivatedBecauseOfYourCountryChoice');
		//$this->always_enabled = true;								// If true, can't be disabled

		// Constants
		// List of particular constants to add when module is enabled (key, 'chaine', value, desc, visible, 'current' or 'allentities', deleteonunactive)
		// Example: $this->const=array(0=>array('MUSICAL_MYNEWCONST1','chaine','myvalue','This is a constant to add',1),
		//                             1=>array('MUSICAL_MYNEWCONST2','chaine','myvalue','This is another constant to add',0, 'current', 1)
		// );
		$this->const = array(
			1=>array('MUSICAL_MYCONSTANT', 'chaine', 'avalue', 'This is a constant to add', 1, 'allentities', 1)
		);
		...
		if (! isset($conf->musical) || ! isset($conf->musical->enabled))
		{
			$conf->musical=new stdClass();
			$conf->musical->enabled=0;
		}
        ...
        $this->dictionaries=array(
            ...
            'tabcond'=>array($conf->musical->enabled,$conf->musical->enabled,$conf->musical->enabled)												// Condition to show each dictionary
        );
        */


        // Boxes/Widgets
		// Add here list of php file(s) stored in musical/core/boxes that contains class to show a widget.
        $this->boxes = array(
		...
		$this->cronjobs = array(
			0=>array('label'=>'MyJob label', 'jobtype'=>'method', 'class'=>'/musical/class/instrument.class.php', 'objectname'=>'instrument', 'method'=>'doScheduledJob', 'parameters'=>'', 'comment'=>'Comment', 'frequency'=>2, 'unitfrequency'=>3600, 'status'=>0, 'test'=>'$conf->musical->enabled', 'priority'=>50)
		);
		// Example: $this->cronjobs=array(0=>array('label'=>'My label', 'jobtype'=>'method', 'class'=>'/dir/class/file.class.php', 'objectname'=>'MyClass', 'method'=>'myMethod', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>2, 'unitfrequency'=>3600, 'status'=>0, 'test'=>'$conf->musical->enabled', 'priority'=>50),
		//                                1=>array('label'=>'My label', 'jobtype'=>'command', 'command'=>'', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>1, 'unitfrequency'=>3600*24, 'status'=>0, 'test'=>'$conf->musical->enabled', 'priority'=>50)
		// );
		...
		$this->rights[$r][1] = 'Read instrument of Musical';	// Permission label
		$this->rights[$r][3] = 1; 					// Permission by default for new user (0/1)
		$this->rights[$r][4] = 'read';				// In php code, permission will be checked by test if ($user->rights->musical->level1->level2)
		$this->rights[$r][5] = '';				    // In php code, permission will be checked by test if ($user->rights->musical->level1->level2)

		$r++;
		$this->rights[$r][0] = $this->numero + $r;	// Permission id (must not be already used)
		$this->rights[$r][1] = 'Create/Update instrument of Musical';	// Permission label
		$this->rights[$r][3] = 1; 					// Permission by default for new user (0/1)
		$this->rights[$r][4] = 'write';				// In php code, permission will be checked by test if ($user->rights->musical->level1->level2)
		$this->rights[$r][5] = '';				    // In php code, permission will be checked by test if ($user->rights->musical->level1->level2)

		$r++;
		$this->rights[$r][0] = $this->numero + $r;	// Permission id (must not be already used)
		$this->rights[$r][1] = 'Delete instrument of Musical';	// Permission label
		$this->rights[$r][3] = 1; 					// Permission by default for new user (0/1)
		$this->rights[$r][4] = 'delete';				// In php code, permission will be checked by test if ($user->rights->musical->level1->level2)
		$this->rights[$r][5] = '';				    // In php code, permission will be checked by test if ($user->rights->musical->level1->level2)
		...
		*/
		...
		/* END MODULEBUILDER LEFTMENU INSTRUMENT */


		// Exports
		$r=1;

		/* BEGIN MODULEBUILDER EXPORT INSTRUMENT */
		/*
		$langs->load("musical@musical");
		$this->export_code[$r]=$this->rights_class.'_'.$r;
		$this->export_label[$r]='instrumentLines';	// Translation key (used only if key ExportDataset_xxx_z not found)
		$this->export_icon[$r]='instrument@musical';
		$keyforclass = 'instrument'; $keyforclassfile='/mymobule/class/instrument.class.php'; $keyforelement='instrument';
		include DOL_DOCUMENT_ROOT.'/core/commonfieldsinexport.inc.php';
		$keyforselect='instrument'; $keyforaliasextra='extra'; $keyforelement='instrument';
		include DOL_DOCUMENT_ROOT.'/core/extrafieldsinexport.inc.php';
		//$this->export_dependencies_array[$r]=array('mysubobject'=>'ts.rowid', 't.myfield'=>array('t.myfield2','t.myfield3')); // To force to activate one or several fields if we select some fields that need same (like to select a unique key if we ask a field of a child to avoid the DISTINCT to discard them, or for computed field than need several other fields)
		$this->export_sql_start[$r]='SELECT DISTINCT ';
		$this->export_sql_end[$r]  =' FROM '.MAIN_DB_PREFIX.'instrument as t';
